...
@tool
class_name InteractAreaNew
extends Area2D

signal interaction_started(character: Character, from_right: bool)
...
## appear when a character is close.
...
func start_interaction(character: Character, from_right: bool) -> void:
	interaction_started.emit(character, from_right)
